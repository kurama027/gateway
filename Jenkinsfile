node {
   ...
   stage('Preparation') {
       git 'https://github.com/kurama027/gateway.git'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/helena-servicio-gateway/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/helena-servicio-gateway/runDeployment.sh'
           sh '/var/lib/jenkins/workspace/helena-servicio-gateway/runDeployment.sh'
       }catch (e){
